  ...
  def run
    ...
    SNMP::Manager.open(host: "#{config[:host]}", community: "#{config[:community]}", version: config[:version]) do |manager|
      manager.walk(if_table_columns) do |row_array|
        ...
        if_name = config[:include_name] ? "#{row['ifIndex'].value}_#{graphite_safe_name(row['ifName'].value.to_s)}" : row['ifIndex'].value.to_s

#        next if row['ifOperStatus'].value != 1 && !config[:include_down_interfaces] 
        next if row['ifOperStatus'].value != 1
        in_octets = config[:low_capacity] ? 'ifInOctets' : 'ifHCInOctets'
        out_octets = config[:low_capacity] ? 'ifOutOctets' : 'ifHCOutOctets'
        output "#{if_name}"'_ifIn_Octets,'"source=#{config[:scheme]},interface=#{if_name}",row[in_octets].value if row['ifHCInOctets'].value.to_s != 'noSuchInstance'
        output "#{if_name}"'_ifOut_Octets,'"source=#{config[:scheme]},interface=#{if_name}",row[out_octets].value if row['ifHCOutOctets'].value.to_s != 'noSuchInstance'
