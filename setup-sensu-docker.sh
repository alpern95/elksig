...
generate_ssl() {
  ...
  cd ../sensu_ca
  openssl ca -config $workdir/support/openssl.cnf -in ../server/req.pem -out ../server/cert.pem -notext -batch -extensions server_ca_extens
ions
  cd ../server
  ...
  cd ../sensu_ca
  openssl ca -config $workdir/support/openssl.cnf -in ../client/req.pem -out ../client/cert.pem -notext -batch -extensions client_ca_extens
ions
  cd ../client
  openssl pkcs12 -export -out keycert.p12 -in cert.pem -inkey key.pem -passout pass:$passwd
  cd ../
